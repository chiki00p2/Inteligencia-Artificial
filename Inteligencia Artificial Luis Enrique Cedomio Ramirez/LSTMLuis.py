...
# Comprobar si CUDA está disponible y seleccionar el dispositivo
...
plt.plot(np.arange(x.shape[1]), y[0, :], 'r', linewidth=2.0, label='Input Data')
...
if __name__ == "__main__":
    ...
    for i in range(n_steps):
        ...
        plt.figure(figsize=(12, 6))
        plt.title(f"Prediction Step {i+1}", fontsize=16)
        plt.xlabel("x", fontsize=14)
        plt.ylabel("y", fontsize=14)
        plt.xticks(fontsize=12)
        plt.yticks(fontsize=12)
        ...
        def draw(y_i, color):
            plt.plot(np.arange(n), y_i[0, :n], color, linewidth=2.0, label='Training Prediction')
            plt.plot(np.arange(n, n+future), y_i[0, n:], color + ":", linewidth=2.0, label='Future Prediction')
